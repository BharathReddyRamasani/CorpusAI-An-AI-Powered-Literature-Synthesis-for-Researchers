...
# ── POST /api/chat/section ───────────────────────────────────────────────────

from app.schemas.research import SectionChatRequest

@router.post(
    "/section",
    response_model=ChatResponse,
    summary="Ask a question about a specific section",
)
async def ask_section_question(
    ...
    """Chat with a specific section of a paper."""
    answer, sources = await rag_service.answer_question(
        db=db,
        paper_id=payload.paper_id,
        user_id=current_user.id,
