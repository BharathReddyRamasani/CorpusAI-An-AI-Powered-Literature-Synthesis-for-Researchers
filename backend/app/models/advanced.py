...
class ResearchInsight(Base):
    ...
    paper_id: str = Column(String(64), ForeignKey("papers.paper_id", ondelete="CASCADE"), nullable=False, index=True, unique=True)
...
class SharedDashboard(Base):
    ...
    user_id: int = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    snapshot_data: str = Column(JSON, nullable=False)
    created_at: datetime = Column(DateTime, server_default=func.now(), nullable=False)
